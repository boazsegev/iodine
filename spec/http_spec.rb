...
module TestHTTPHandler
  def self.on_http(e)
    ...
    when '/close_body'
      before_len = e.length
      first_read = e.read(5).to_s
      ret = e.close_body
      e.finish("before=#{before_len} first=#{first_read} after=#{e.length} " \
               "post_read=#{e.read.inspect} self=#{ret.equal?(e)}")
...
RSpec.describe 'Iodine HTTP server' do
  ...
  before(:context) do
    ...
    run_tests = proc do
      ...
      # POST /native/close_body - small (RAM buffered) body released mid-request
      resp = Net::HTTP.post(URI("#{base}/native/close_body"), 'x' * 300, 'Content-Type' => 'text/plain')
      HTTP_RESULTS[:close_body_status] = resp.code.to_i
      HTTP_RESULTS[:close_body]        = resp.body

      # POST /native/close_body - large body (> 128KiB RAM limit, spills to a temp file)
      resp = Net::HTTP.post(URI("#{base}/native/close_body"), 'y' * (200 * 1024), 'Content-Type' => 'text/plain')
      HTTP_RESULTS[:close_body_fd_status] = resp.code.to_i
      HTTP_RESULTS[:close_body_fd]        = resp.body
  ...
  # -------------------------------------------------------------------------
  # close_body (Iodine::Connection#close_body)
  # -------------------------------------------------------------------------
  describe 'Iodine::Connection#close_body' do
    it 'returns status 200 for a RAM-buffered body' do
      expect(HTTP_RESULTS[:close_body_status]).to eq(200)
    end

    it 'releases a RAM-buffered body' do
      expect(HTTP_RESULTS[:close_body]).to include('before=300')
      expect(HTTP_RESULTS[:close_body]).to include('first=xxxxx')
      expect(HTTP_RESULTS[:close_body]).to include('after=0')
    end

    it 'subsequent reads return nil' do
      expect(HTTP_RESULTS[:close_body]).to include('post_read=nil')
    end

    it 'returns self' do
      expect(HTTP_RESULTS[:close_body]).to include('self=true')
    end

    it 'returns status 200 for a file-backed body' do
      expect(HTTP_RESULTS[:close_body_fd_status]).to eq(200)
    end

    it 'releases a file-backed (temp file) body' do
      expect(HTTP_RESULTS[:close_body_fd]).to include("before=#{200 * 1024}")
      expect(HTTP_RESULTS[:close_body_fd]).to include('after=0')
      expect(HTTP_RESULTS[:close_body_fd]).to include('post_read=nil')
    end
  end
end
