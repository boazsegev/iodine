...
#      ws = new WebSocket("ws://localhost:3000"); ws.onmessage = function(e) {console.log(e.data);}; ws.onclose = function(e) {console.log("closed")};
...
module TaskHandler
  def echo msg
    return unless Iodine.master? # don't work more than once...
    msg = Iodine::JSON.parse(msg, symbolize_names: true)
    publish_to = msg.delete(:from)
    Iodine.publish(publish_to, msg.to_json) if publish_to
...
module WebsocketClient
  ...
  def on_message client, data
    ...
    client.publish msg[:task], msg.to_json
  rescue => e
