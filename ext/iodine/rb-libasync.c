...
// join a ruby thread
static void* join_thread(THREAD_TYPE thr) {
  return rb_thread_call_with_gvl(_inner_join_with_rbthread, (void*)thr);
}

// /** POSIX version: create thread  */
...
static VALUE thread_loop(void* _args) {
  ...
  void* async = args->async;
  free(_args);
  rb_thread_call_without_gvl2(thread_func, async, (void (*)(void*))async_signal,
                              async);
  return Qnil;
}
...
static int create_thread(THREAD_TYPE* thr,
  ...
  if (!data)
    return -1;
  *data = (struct CreateThreadArgs){.thread_func = thread_func, .async = async};
