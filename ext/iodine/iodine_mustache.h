...
static void fio_mustache_wrapper_free(void *p) {
  fio_mustache_wrapper_s *wrapper = (fio_mustache_wrapper_s *)p;
...
static VALUE mus_render(VALUE self, VALUE ctx) {
  ...
  char *result = fio_mustache_build(wrapper->m,
                                    .get_var = mus_get_var,
...
*/
static void Init_iodine_musta(void) { // clang-format on
  VALUE m = rb_define_class_under(iodine_rb_IODINE, "Mustache", rb_cObject);
