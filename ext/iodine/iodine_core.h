...
static void *iodine___run(void *ignr_) {
  ...
  unsigned threads = (unsigned)fio_srv_workers((int)fio_cli_get_i("-t"));
  unsigned workers = (unsigned)fio_srv_workers((int)fio_cli_get_i("-w"));
  fio_srv_async_update(&IODINE_THREAD_POOL, (uint32_t)threads);
...
static VALUE iodine_workers(VALUE klass) {
  if (!fio_cli_get("-w"))
    return Qnil;
  unsigned long tmp = fio_srv_workers((uint16_t)fio_cli_get_i("-w"));
  return LL2NUM(tmp);
  (void)klass;
}
...
static VALUE iodine_threads(VALUE klass) {
  if (!fio_cli_get("-t"))
    return Qnil;
  unsigned long tmp = fio_srv_workers((uint16_t)fio_cli_get_i("-t"));
  return LL2NUM(tmp);
  (void)klass;
}
