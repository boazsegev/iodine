...
static VALUE iodine_connection_unsubscribe_internal(fio_io_s *io,
  ...
  if ((size_t)filter & (~(size_t)0xFFFF))
    rb_raise(rb_eRangeError,
             "filter out of range (%lld > 0xFFFF)",
             (long long)filter);
  return fio_pubsub_unsubscribe(.io = io, .channel = channel, .filter = filter)
             ? RUBY_Qfalse
             : RUBY_Qtrue;
}
 ...
 * - `channel`: the subscription's channel name (String).
 * - `filter`:  the subscription's filter (Number < 32,767).
 * - `since`:   replay cached messages since this timestamp in milliseconds
 *              (Number, requires history to be enabled).
 ...
 *     # with history replay (requires Iodine::PubSub::History)
 *     subscribe(channel: "name", since: Time.now.to_i * 1000 - 60000)
 *
 */
 ...
 *     # with history replay (requires Iodine::PubSub::History.cache)
 *     Iodine.subscribe(channel: "name", since: Time.now.to_i * 1000 - 60000)
 *
 */
