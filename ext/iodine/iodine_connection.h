...
static VALUE iodine_connection_create_from_io(fio_s *io) {
  VALUE m = rb_obj_alloc(iodine_rb_IODINE_CONNECTION);
  STORE.hold(m);
  iodine_connection_s *c = iodine_connection_ptr(m);
  c->store[IODINE_CONNECTION_STORE_handler] = (VALUE)fio_udata_get(io);
  c->io = io;
  c->http = NULL;
  fio_udata_set(io, (void *)m);
  return m;
}
...
static VALUE iodine_connection_is_clean(VALUE o) {
  ...
  if (c->http)
    return (fio_http_is_clean(c->http) ? Qfalse : Qtrue);
  if (c->io)
    return (fio_srv_is_open(c->io) ? Qtrue : Qfalse);
  return Qfalse;
}
...
headers were already sent or either `write` or `finish` previously called, it
**MUST** return `false`.

    * The header `name` **MUST** be a lower case String. Servers **MAY** enforce
this by converting String objects to lower case.

    * The `write_header` method is **irreversible**. Servers **MAY** write the
header immediately, as they see fit.

    * When frameworks provide an interface for setting cookies, they **SHOULD**
consider supporting all the features available for the [`set-cookie`
...
static int iodine_handler_deafult_on_http__header(VALUE n, VALUE v, VALUE h_) {
  fio_http_s *h = (fio_http_s *)h_;
  if (RB_TYPE_P(v, RUBY_T_ARRAY))
    goto is_array;
  if (!RB_TYPE_P(v, RUBY_T_STRING))
    return ST_CONTINUE;
  iodine_connection___add_header(h,
...
static VALUE iodine_handler_deafult_on_http(VALUE handler, VALUE client) {
  ...
  r = c->store[IODINE_CONNECTION_STORE_rack] =
      rb_funcallv(handler, IODINE_CALL_ID, 1, &env);
  if (!c->http)
    return returned_value;
...
static void iodine_io_raw_on_attach(fio_s *io) {
  /* Enter GIL */
...
static void iodine_io_raw_on_data(fio_s *io) {
  iodine_io_raw_on_data_info_s i;
  i.io = io;
  i.len = fio_read(io, i.buf, IODINE_RAW_ON_DATA_READ_BUFFER);
  if (!i.len)
    return;
...
#define IODINE_CONNECTION_DEF_CB(named, id)                                    \
  static void iodine_io_raw_##named(fio_s *io) {                               \
    VALUE connection = (VALUE)fio_udata_get(io);                               \
    if (!connection || connection == Qnil)                                     \
      return;                                                                  \
    iodine_connection_s *c = iodine_connection_ptr(connection);                \
    if (!c)                                                                    \
...
FIO_IFUNC iodine_connection_args_s iodine_connection_parse_args(int argc,
  ...
  if (r.rb_tls != Qnil && r.rb_tls != Qfalse) {
    ...
    r.settings.tls =
        (r.rb_tls == Qtrue ? fio_tls_new()
                           : fio_tls_dup(iodine_tls_get(r.rb_tls)));
    if (r.rb_tls == Qtrue && fio_cli_get("-cert") && fio_cli_get("-key"))
      fio_tls_cert_add(r.settings.tls,
                       fio_cli_get("-name"),
                       fio_cli_get("-cert"),
                       fio_cli_get("-key"),
...
/** Called after the connection was closed (called once per IO). */
static void iodine_io_raw_client_on_close(void *udata) {
  VALUE connection = (VALUE)udata;
  if (!connection || connection == Qnil)
    return;
  iodine_connection_s *c = iodine_connection_ptr(connection);
  if (c) {
    iodine_ruby_call_outside(c->store[IODINE_CONNECTION_STORE_handler],
                             ...
                             1,
                             &connection);
    fio_protocol_s *p = fio_protocol_get(c->io);
...
static VALUE iodine_connection_initialize(int argc, VALUE *argv, VALUE self) {
  ...
  if (args.service == IODINE_SERVICE_HTTP ||
    ...
    fio_protocol_s *protocol = FIO_MEM_REALLOC(NULL, 0, sizeof(*protocol), 0);
    ...
    *protocol = IODINE_RAW_PROTOCOL;
    ...
    c->io = fio_srv_connect(args.url.buf,
                            .protocol = protocol,
...
static VALUE iodine_connection_peer_addr(VALUE self) {
  ...
  iodine_connection_s *c = iodine_connection_ptr(self);
  if (c->io)
    buf = fio_sock_peer_addr(fio_fd_get(c->io));
  if (!buf.len)
    return Qnil;
...
static VALUE iodine_connection_rack_hijack(VALUE self) {
  ...
  if (!c->http)
    return Qnil;
  int new_fd = fio_sock_dup(fio_fd_get(fio_http_io(c->http)));
...
static VALUE iodine_connection_publish_internal(fio_s *io,
                                                int argc,
                                                VALUE *argv,
                                                VALUE self) {
  VALUE message = Qnil;
...
static void *iodine_tcp_listen(iodine_connection_args_s args) {
  ...
  *protocol = IODINE_RAW_PROTOCOL;
  ...
  return fio_srv_listen(.url = args.url.buf,
                        .protocol = protocol,
                        .udata = args.settings.udata,
                        .tls = args.settings.tls,
...
static VALUE iodine_listen_rb(int argc, VALUE *argv, VALUE self) {
  ...
  fio_tls_free(s.settings.tls);
  if (!listener)
    rb_raise(rb_eRuntimeError, "Couldn't open listening socket.");
  if (fio_srv_listener_is_tls(listener))
